...
# Import your existing AdaIN Layer
...
def load_models():
    ...
    if decoder is None:
        ...
        decoder.load_state_dict(
            torch.load("decoder_3.pth", map_location=device)
        )
        decoder.eval()
...
# decoder.load_state_dict(torch.load(r"D:\\PROJECTS\\Neural_Style_Transfer\\experiment\\train2\\decoder 10.pth"))
# decoder.load_state_dict(
#     torch.load("decoder_10.pth", map_location=device)
# )
# encoder.eval()
# decoder.eval()
...
def index():
   ...
   if form.validate_on_submit():
      ...
      if not content_filename:
         error = "Please upload content image to proceed"
      if not style_filename:
         error = "Please upload style image to proceed"
