...
def adaptive_instance_normalization(c_feats, s_feats):
  # input format is : [batch_size, channels, h, w]
  size = c_feats.size()
  style_mean , style_std = calc_mean_std(s_feats)
  content_mean, content_std = calc_mean_std(c_feats)
  normalize_content_feats = (c_feats - content_mean.expand(size))/(content_std.expand(size))
  
  return (normalize_content_feats * style_std.expand(size)) + style_mean.expand(size)
